...
export default async function handler(req, res) {
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({
      // gemini-2.0-flash (shut down June 1, 2026) and then gemini-2.5-flash
      // (no longer available to new users, per Google's own error message)
      // both stopped working. Google's whole lineup has moved to the 3.x
      // family. Pinned to gemini-3.1-flash-lite: it's still free-tier
      // eligible (only Pro-tier models went paid-only), the cheapest paid
      // rate if billing ever gets turned on, and a stable GA release rather
      // than a preview build that can vanish without warning. No official
      // shutdown date announced yet as of July 2026 — still, check
      // https://ai.google.dev/gemini-api/docs/deprecations occasionally and
      // update this string when Google eventually moves it.
