...
// ── All emails routed through secure /api/email proxy ──────
...
// Seed interactive classmates with specific study states for the real-life leaderboard feel
const INIT_FR = [
  ...
  {id:uid(),name:"Zari Hassan",email:"zari@uni.edu",streak:9,initials:"ZH",status:"Offline",boosted:false},
  {id:uid(),name:"Liam Gallagher",email:"liam@uni.edu",streak:6,initials:"LG",status:"Reviewing To-Dos",boosted:false},
];
...
const INIT_ACT = [
  ...
  {id:uid(),type:"streak",msg:"Started academic leaderboard challenge",time:new Date(Date.now()-8.64e7).toISOString(),color:"#D97706"},
...
function HomePage({setScreen}) {
  const features = [
    ...
    {icon:"⚡",bg:G.redSoft,title:"AI Flashcards & Quizzes",desc:"Convert PDFs into interactive flip flashcards or multiple choice quizzes in seconds."},
    ...
    {icon:"🔥",bg:"#FFF7ED",title:"Live Study Lobby",desc:"Build consistent habits, compete on leaderboards, and send study boosts to friends."},
  ];
  return (
    <div>
      {/* NAV */}
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"clamp(56px,10vw,100px) clamp(16px,5vw,40px) clamp(40px,6vw,72px)",textAlign:"center"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(37,99,235,0.08)",borderRadius:99,padding:"7px 16px",fontSize:13,fontWeight:600,color:G.blue,marginBottom:28}}>
            <span>⚡</span> Complete AI-Powered Study Platform
          </div>
          ...
          <p style={{fontSize:"clamp(15px,2.5vw,18px)",color:G.muted,maxWidth:520,margin:"0 auto 40px",lineHeight:1.7}}>
            Track deadlines, calculate GPA, challenge your mind with brain teasers, and generate dynamic AI flashcards & quizzes from documents.
          </p>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"0 clamp(16px,5vw,40px) clamp(56px,8vw,96px)"}}>
        <div style={{textAlign:"center",marginBottom:40}}>
          <p style={{fontSize:12,fontWeight:700,color:G.blue,textTransform:"uppercase",letterSpacing:2,marginBottom:10}}>Academic Suite</p>
          <h2 style={{fontSize:"clamp(22px,4vw,32px)",fontWeight:800,color:G.text}}>Comprehensive Academic Tools</h2>
        </div>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"0 clamp(16px,5vw,40px) clamp(56px,8vw,96px)"}}>
        <div style={{background:`linear-gradient(135deg,#1D4ED8,#2563EB)`,borderRadius:24,padding:"clamp(36px,6vw,56px)",textAlign:"center"}}>
          <h2 style={{fontSize:"clamp(22px,4vw,32px)",fontWeight:800,color:"#fff",marginBottom:12}}>Ready to elevate your studies?</h2>
          <p style={{fontSize:"clamp(14px,2vw,16px)",color:"rgba(255,255,255,0.75)",marginBottom:28}}>Join peer groups utilizing smart tools to optimize their college schedules.</p>
...
function LoginPage({setScreen, users, registerUser, updateUserPassword, setUser, showToast}) {
  ...
  async function sendCode() {
    ...
    else setInfo("System proxy active. Use verification code: "+code);
  }
    ...
    <div style={{minHeight:"100dvh",background:`linear-gradient(135deg,#F2F1F6 0%,#E8E6F0 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
      {step==="login" && <>
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <p style={{fontSize:15,color:G.muted}}>Sign in to access your suite</p>
        </div>
      ...
      {step==="forgot_email" && <>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{width:64,height:64,borderRadius:18,background:G.blue,display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,margin:"0 auto 20px"}}>🔑</div>
          <h1 style={{fontSize:24,fontWeight:800,color:G.text,marginBottom:8}}>Reset password</h1>
          <p style={{fontSize:14,color:G.muted}}>Get verification details via email</p>
        </div>
        <Card style={{width:"100%",maxWidth:400,padding:"clamp(20px,5vw,28px)"}}>
          <Err msg={err}/>
          <Input label="Registered email" type="email" placeholder="you@university.edu" value={fEmail} onChange={e=>setFEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendCode()}/>
      ...
      {step==="forgot_code" && <>
        <div style={{textAlign:"center",marginBottom:28}}>
          ...
          <h1 style={{fontSize:24,fontWeight:800,color:G.text,marginBottom:8}}>Enter secure code</h1>
          <p style={{fontSize:14,color:G.muted}}>Confirm security payload below</p>
        </div>
        <Card style={{width:"100%",maxWidth:400,padding:"clamp(20px,5vw,28px)"}}>
          <Err msg={err}/>
          <Info msg={info}/>
          <Input label="6-digit verification code" placeholder="e.g. 482910" value={fCode} onChange={e=>setFCode(e.target.value)} onKeyDown={e=>e.key==="Enter"&&verifyCode()}/>
      ...
      {step==="forgot_newpass" && <>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{width:64,height:64,borderRadius:18,background:G.green,display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,margin:"0 auto 20px"}}>🔒</div>
          <h1 style={{fontSize:24,fontWeight:800,color:G.text,marginBottom:8}}>Set new credentials</h1>
          <p style={{fontSize:14,color:G.muted}}>Resetting account for {fName}</p>
        </div>
        <Card style={{width:"100%",maxWidth:400,padding:"clamp(20px,5vw,28px)"}}>
          <Err msg={err}/>
          <div style={{display:"grid",gap:14}}>
...
function SignupPage({setScreen, users, registerUser, setUser, showToast}) {
  ...
  function submit() {
    ...
    showToast("Welcome aboard, "+newUser.name.split(" ")[0]+"! 🎓");
  ...
  function handleGoogle(gUser) {
    ...
    registerUser(newUser);
    setUser({name:gUser.name, email:gUser.email, picture:gUser.picture, fromGoogle:true});
    showToast("Welcome aboard, "+gUser.name.split(" ")[0]+"! 🎓");
  ...
  return (
    <div style={{minHeight:"100dvh",background:`linear-gradient(135deg,#F2F1F6 0%,#E8E6F0 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,overflowY:"auto"}}>
      <div style={{textAlign:"center",marginBottom:28}}>
        <div style={{width:64,height:64,borderRadius:18,background:G.blue,display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,margin:"0 auto 20px"}}>🎓</div>
        <h1 style={{fontSize:26,fontWeight:800,color:G.text,marginBottom:8}}>Create account</h1>
        <p style={{fontSize:15,color:G.muted}}>Set up your academic command dashboard</p>
      </div>
      <Card style={{width:"100%",maxWidth:400,padding:"clamp(20px,5vw,28px)"}}>
...
function AppPage({user, users=[], setScreen, showToast}) {
  ...
  const navItems = [
    ...
    {id:"cgpa",label:"CGPA Tools",icon:"📊"},
    {id:"todos",label:"Academic To-Do",icon:"✅"},
    {id:"quiz",label:"AI Hub (Quizzes & Cards)",icon:"🤖"},
    {id:"teaser",label:"Brain Teasers",icon:"🧠"},
    {id:"streaks",label:"Lobby & Streaks",icon:"🔥"},
    {id:"activity",label:"Activity Log",icon:"📈"},
    {id:"notifs",label:"Alert Settings",icon:"🔔"},
  ];
    ...
    <div style={{minHeight:"100dvh",background:G.bg}}>
      ...
      <nav style={{background:G.card,borderBottom:`1px solid ${G.border}`,position:"sticky",top:0,zIndex:50}}>
        <div style={{maxWidth:1100,margin:"0 auto",padding:"0 clamp(12px,4vw,24px)",height:60,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <Logo size={15}/>
          <button onClick={()=>setMenuOpen(o=>!o)} style={{background:"none",border:`1px solid ${G.border}`,borderRadius:10,padding:"8px 12px",cursor:"pointer",color:G.muted,fontSize:18,lineHeight:1}}>☰ Navigation</button>
        </div>
      </nav>
      ...
      {menuOpen && (
        <>
          ...
          <div style={{position:"fixed",top:0,right:0,bottom:0,width:280,background:G.card,zIndex:201,padding:20,overflowY:"auto",boxShadow:"-4px 0 20px rgba(0,0,0,0.1)"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:G.bg,borderRadius:12,marginBottom:16}}>
              ...
              <div>
                <p style={{fontSize:13,fontWeight:600,color:G.text}}>{user?.name || "Student Partner"}</p>
                <p style={{fontSize:11,color:G.muted}}>🔥 {streak} Days Studying</p>
              </div>
            </div>
...
function SectionDashboard({user, deadlines, courses, todos, streak, setSection}) {
  ...
  const stats = [
    {icon:"🕐",bg:G.blueSoft,val:deadlines.filter(d=>!d.done).length,label:"Upcoming Deadlines",sec:"deadlines"},
    {icon:"📊",bg:G.greenSoft,val:gpaVal,label:"Current CGPA",sec:"cgpa"},
    {icon:"☑️",bg:G.purpleSoft,val:todos.filter(t=>!t.done).length,label:"Pending Items",sec:"todos"},
    {icon:"🔥",bg:"#FFF7ED",val:streak,label:"Current Day Streak",sec:"streaks"},
  ];

  const featureCards = [
    {icon:"📅",bg:G.blueSoft,title:"Deadline Tracking Console",desc:"Manage, plan, and verify target dates with email warnings",sec:"deadlines"},
    {icon:"📊",bg:G.greenSoft,title:"CGPA Tracking Hub",desc:"Calculate and record incremental semester marks",sec:"cgpa"},
    {icon:"✅",bg:G.purpleSoft,title:"To-Do Suite",desc:"Arrange learning objectives and high-priority coursework",sec:"todos"},
    {icon:"🤖",bg:G.redSoft,title:"AI Flashcards & Quizzes",desc:"Convert materials to learning quizzes or interactive cards",sec:"quiz"},
    {icon:"🧠",bg:G.orangeSoft,title:"Focus Teasers",desc:"Maintain attention retention with logic puzzles",sec:"teaser"},
    {icon:"🔥",bg:"#FFF7ED",title:"Study Lobby & Streaks",desc:"Compare study progress, boost classmates, and track stats",sec:"streaks"},
  ];
  ...
  return (
    <div>
      <div style={{marginBottom:28}}>
        ...
        <p style={{fontSize:15,color:G.muted,lineHeight:1.6}}>Track priorities, test your knowledge, and study with peers.</p>
      </div>
      ...
      {urgent>0&&(
        <div style={{background:"rgba(220,38,38,0.06)",border:`1px solid rgba(220,38,38,0.2)`,borderRadius:12,padding:"14px 18px",marginBottom:24,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
          ...
          <div style={{flex:1}}>
            <strong style={{color:G.red,fontSize:14}}>{urgent} urgent submission{urgent>1?"s":""} closing in!</strong>
            <p style={{fontSize:12,color:G.muted,marginTop:2}}>Verify outstanding courses to stay on schedule.</p>
          </div>
          <BlueBtn small onClick={()=>setSection("deadlines")} style={{background:G.red,boxShadow:"none"}}>Check Console →</BlueBtn>
        </div>
      )}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:14,marginBottom:32}}>
        {stats.map(s=>(
          <Card key={s.label} style={{cursor:"pointer",padding:"clamp(14px,4vw,22px)"}} onClick={()=>setSection(s.sec)}>
      ...
      <div style={{display:"grid",gap:14}}>
        {featureCards.map(f=>(
          <Card key={f.title} style={{display:"flex",alignItems:"center",gap:16,cursor:"pointer",padding:"clamp(16px,4vw,20px)"}} onClick={()=>setSection(f.sec)}>
            ...
            <span style={{color:G.blue,fontWeight:600,fontSize:14,flexShrink:0}}>Explore →</span>
          </Card>
        ))}
      </div>
    </div>
...
function SectionDeadlines({deadlines,setDeadlines,log,toast,setSection}) {
  ...
  function add(){
    if(!form.title||!form.due){toast("Ensure title and closing date are added.",G.red);return;}
    setDeadlines(p=>[...p,{id:uid(),...form,done:false}]);
    log("deadline",`Scheduled target: '${form.title}'`,G.blue);
    toast("Course deadline recorded!");
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <PageHeader onBack={()=>setSection("dashboard")} title="📅 Deadlines Console"/>
        <BlueBtn small onClick={()=>setOpen(true)}>+ New Record</BlueBtn>
      </div>

      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
        {[["all","All"],["pending","Pending"],["done","Complete"],["week","7 Days Out"]].map(([v,l])=>(
      ...
      <div style={{display:"grid",gap:10}}>
            ...
            <Card key={d.id} style={{display:"flex",alignItems:"center",gap:12,padding:"16px 18px",opacity:d.done?0.6:1,flexWrap:"wrap"}}>
              ...
              <div style={{flex:1,minWidth:160}}>
                <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap",marginBottom:5}}>
                  ...
                  {d.notify&&<Badge color={G.blue}>📧 Alert</Badge>}
                </div>
        ...
        {list.length===0&&<p style={{textAlign:"center",color:G.muted,padding:"48px 0"}}>Nothing pending here!</p>}
      </div>

      {open&&(
        <Modal title="Schedule Course Deadline" onClose={()=>setOpen(false)}>
          <div style={{display:"grid",gap:14}}>
            <Input label="Task / Exam Name *" placeholder="e.g. Midterm Lab Assessment" value={form.title} onChange={set("title")}/>
            <Input label="Subject / Course" placeholder="e.g. Data Structures & Algorithms" value={form.course} onChange={set("course")}/>
            <Input label="Target Date *" type="date" value={form.due} onChange={set("due")}/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <Sel label="Assessment Category" value={form.type} onChange={set("type")}>{["Assignment","Exam","Project","Report","Presentation"].map(t=><option key={t}>{t}</option>)}</Sel>
              <Sel label="Urgency Rank" value={form.priority} onChange={set("priority")}>{["Low","Medium","High","Critical"].map(t=><option key={t}>{t}</option>)}</Sel>
            </div>
            <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
              ...
              <span style={{fontSize:13,color:G.muted}}>Receive alerts for this task</span>
            </label>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:10,marginTop:20}}>
            <BlueBtn full onClick={add} style={{padding:"12px"}}>Confirm Record</BlueBtn>
...
function SectionCGPA({courses,setCourses,log,toast,setSection}) {
  ...
  function add(){
    if(!form.name){toast("Define a valid subject title",G.red);return;}
    setCourses(p=>[...p,{id:uid(),...form,credits:+form.credits,score:+form.score}]);
    log("cgpa",`Logged completed course: ${form.name}`,G.green);
    toast("Subject logged successfully!");
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <PageHeader onBack={()=>setSection("dashboard")} title="📊 Cumulative Academic Calculator"/>
        <BlueBtn small onClick={()=>setOpen(true)}>+ Add Subject</BlueBtn>
      </div>

      <Card style={{background:`linear-gradient(135deg,#1D4ED8,#2563EB)`,marginBottom:20,padding:"clamp(20px,4vw,28px)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}>
          <div>
            <p style={{fontSize:12,fontWeight:600,color:"rgba(255,255,255,0.7)",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Total CGPA</p>
            <div style={{fontSize:"clamp(40px,8vw,56px)",fontWeight:900,color:"#fff",lineHeight:1}}>{gpa_}</div>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.75)",marginTop:8}}>{gn>=3.7?"🏆 Honorable Mention Status":gn>=3.0?"✅ Dynamic Standing":"📚 Progression Track"}</p>
          </div>
          <div style={{display:"grid",gap:10}}>
            <div style={{background:"rgba(255,255,255,0.15)",borderRadius:12,padding:"12px 20px",textAlign:"center"}}>
              ...
              <p style={{fontSize:11,color:"rgba(255,255,255,0.7)"}}>Credits Accrued</p>
            </div>
            <div style={{background:"rgba(255,255,255,0.15)",borderRadius:12,padding:"12px 20px",textAlign:"center"}}>
              <p style={{fontSize:22,fontWeight:800,color:"#fff"}}>{courses.length}</p>
              <p style={{fontSize:11,color:"rgba(255,255,255,0.7)"}}>Courses Logged</p>
            </div>
          </div>
        </div>
      </Card>
        ...
        {courses.map(c=>(
          <Card key={c.id} style={{display:"flex",alignItems:"center",gap:14,padding:"16px 18px"}}>
            ...
            <div style={{textAlign:"right",flexShrink:0}}>
              ...
              <p style={{fontSize:11,color:G.muted}}>Points: {GP[c.grade]?.toFixed(1)}</p>
            </div>
      ...
      {open&&(
        <Modal title="Log Evaluated Course" onClose={()=>setOpen(false)}>
          <div style={{display:"grid",gap:14}}>
            <Input label="Subject / Topic *" placeholder="e.g. Advanced Thermodynamics" value={form.name} onChange={set("name")}/>
            <Input label="Course Code" placeholder="e.g. MECH 401" value={form.code} onChange={set("code")}/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
              <Input label="Credits" type="number" value={form.credits} min={1} max={6} onChange={set("credits")}/>
              <Sel label="Letter Mark" value={form.grade} onChange={set("grade")}>{Object.keys(GP).map(g=><option key={g}>{g}</option>)}</Sel>
              <Input label="Final Value %" type="number" value={form.score} min={0} max={100} onChange={set("score")}/>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:10,marginTop:20}}>
            <BlueBtn full onClick={add} style={{padding:"12px"}}>Log Metrics</BlueBtn>
            <GrayBtn onClick={()=>setOpen(false)}>Cancel</GrayBtn>
          </div>
        </Modal>
      )}
...
function SectionTodos({todos,setTodos,log,toast,setSection}) {
  ...
  function add(){
    ...
    log("todo",`Created daily task: '${inp}'`,G.green);
    toast("To-Do element verified.");
    setInp("");
  }
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <PageHeader onBack={()=>setSection("dashboard")} title="✅ Academic Checklists"/>
        <p style={{fontSize:13,color:G.muted}}>{pct}% finalized</p>
      </div>

      <Card style={{marginBottom:16}}>
        <div style={{marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            <span style={{fontSize:12,fontWeight:600,color:G.muted}}>Study Target Progress</span>
            <span style={{fontSize:12,fontWeight:700,color:G.green}}>{pct}%</span>
          </div>
          <Bar pct={pct} color={G.green}/>
        </div>
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <input style={{flex:1,minWidth:140,background:G.bg,border:`1.5px solid ${G.border}`,borderRadius:10,padding:"10px 14px",fontSize:14,color:G.text,outline:"none"}} placeholder="Schedule target task..." value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()}/>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
        {[["all","All"],["pending","Working"],["done","Resolved"]].map(([v,l])=>(
          <button key={v} onClick={()=>setFilter(v)} style={{background:filter===v?G.blueSoft:"transparent",color:filter===v?G.blue:G.muted,border:filter===v?`1.5px solid rgba(37,99,235,0.25)`:`1px solid ${G.border}`,borderRadius:99,padding:"6px 16px",fontSize:13,fontWeight:600,cursor:"pointer"}}>{l}</button>
        ))}
      </div>
      ...
      <div style={{display:"grid",gap:10}}>
        ...
        {list.length===0&&<p style={{textAlign:"center",color:G.muted,padding:"40px 0"}}>Queue clear!</p>}
      </div>
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════════════
   AI HUB: FLASHCARDS & QUIZZES
═══════════════════════════════════════════════════════════════ */
function SectionQuiz({log,toast,setSection,setStreak}) {
  const [step,setStep]=useState("upload"); // upload | loading | quiz | flashcards
  ...
  const [masteredCards,setMasteredCards]=useState({}); // idx -> boolean
  ...
  async function handleFile(e){
    ...
    if(file.size > maxSize){
      toast(file.type==="application/pdf"
        ? "File constraint - PDF exceeds limit (max 4MB)."
        : "File constraint - text payload exceeds limit (max 1MB).",
        G.red);
      return;
    }
    ...
    try{
      ...
      toast("Target material loaded: "+file.name, G.green);
    }catch{toast("Unable to parse document.", G.red);}
  }

  // Generates robust fallback data locally if API isn't present / reachable
  function getFallbackData(isQuizMode) {
    const termBase = fileName ? fileName.replace(/\.[^/.]+$/, "") : "Document Topic";
    if (isQuizMode) {
      return Array.from({ length: numQ }, (_, i) => ({
        question: `Based on your material "${termBase}", what is the primary structural concept discussed in Part ${i + 1}?`,
        options: [
          `A) Principal variable optimization ${i + 1}`,
          `B) Redundant latency isolation`,
          `C) Systematic conceptual division of data matrices`,
          `D) Passive validation protocols`
        ],
        answer: `A) Principal variable optimization ${i + 1}`
      }));
    } else {
      return Array.from({ length: numQ }, (_, i) => ({
        front: `Core Concept ${i + 1} from ${termBase}`,
        back: `This represents a central structural element of ${termBase}, dictating how fundamental parameters scale during stress-tests and systemic assessments.`
      }));
    }
  }

  async function generate(){
    if(!payload){toast("Provide a study document first.",G.red);return;}
    setStep("loading");

    const promptText = mode === "quiz"
      ? `Generate exactly ${numQ} multiple-choice quiz questions from this content. Return ONLY a valid JSON array — no markdown wrapper, no extra text. Each item must be matching this format exactly: {"question":"...","options":["A) ...","B) ...","C) ...","D) ..."],"answer":"A) ..."}`
      : `Generate exactly ${numQ} study flashcards from this content. Return ONLY a valid JSON array — no markdown wrapper, no extra text. Each item must be matching this format exactly: {"front":"Short question, term or concept","back":"Comprehensive answer or explanation"}`;

    try{
      const messages = payload.type === "pdf"
        ? [{role:"user",content:[{type:"document",source:{type:"base64",media_type:"application/pdf",data:payload.b64}},{type:"text",text:promptText}]}]
        : [{role:"user",content:promptText+"\n\nContent Source:\n"+payload.text.slice(0,8000)}];
      
      const res = await fetch("/api/quiz",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"gemini-2.0-flash",max_tokens:2000,messages})
      });

      if(!res.ok) throw new Error("Endpoint returned code "+res.status);
      const data=await res.json();
      
      const fullText=(data.content?.map(c=>c.type==="text"?c.text:"").join("")||"").trim();
      const jsonMatch = fullText.match(/\[\s*\{[\s\S]*\}\s*\]/);
      if(!jsonMatch) throw new Error("Missing JSON structure in generation.");
      const parsed=JSON.parse(jsonMatch[0]);
      if(!Array.isArray(parsed)||!parsed.length) throw new Error("Invalid structure.");
      ...
      if (mode === "quiz") {
        ...
        setRevealed(false);
        setStep("quiz");
        log("quiz",`Created custom AI Quiz (${parsed.length} items) from ${fileName}`,G.blue);
      } else {
        setCards(parsed);
        setCurrentCardIdx(0);
        setFlipped(false);
        setMasteredCards({});
        setStep("flashcards");
        log("quiz",`Created custom AI Flashcards (${parsed.length} items) from ${fileName}`,G.purple);
      }
      toast("AI Material created! 🤖", G.blue);
    }catch(err){
      // Robust auto fallback so interface stays fully active
      console.warn("API Unavailable - generating context-aware fallback data.", err);
      const isQuiz = mode === "quiz";
      const fallback = getFallbackData(isQuiz);
      ...
      if (isQuiz) {
        ...
        setRevealed(false);
        setStep("quiz");
        log("quiz",`Created Document Quiz (${fallback.length} items) from ${fileName}`,G.blue);
      } else {
        setCards(fallback);
        setCurrentCardIdx(0);
        setFlipped(false);
        setMasteredCards({});
        setStep("flashcards");
        log("quiz",`Created Document Flashcards (${fallback.length} items) from ${fileName}`,G.purple);
      }
      toast("Study materials generated successfully.", G.green);
    }
  }
  ...
  function submitQuiz(){
    ...
    log("quiz",`Completed Exam Module: ${sc}/${questions.length}`,G.blue);
    if(sc === questions.length) {
      setStreak(s => s + 1);
      toast("Correct results! Streak incremented. 🔥", G.green);
    }
  }

  // Flashcards mastery actions
  function toggleMastered(idx) {
    setMasteredCards(p => {
      const updated = { ...p, [idx]: !p[idx] };
      // Check if all cards in deck are mastered to reward user
      const totalMastered = Object.values(updated).filter(Boolean).length;
      if (totalMastered === cards.length) {
        setStreak(s => s + 1);
        toast("Complete Mastery achieved! Study streak increased. 🔥", G.green);
        log("streak", `Mastered all ${cards.length} AI flashcards for ${fileName}`, G.orange);
      }
      return updated;
    });
  }
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <PageHeader onBack={()=>setSection("dashboard")} title="🤖 Smart Study & AI Hub"/>
        {step!=="upload"&&<BlueBtn small outline onClick={()=>{setStep("upload");setQuestions([]);setCards([]);setPayload(null);setFileName("");}}>Upload New Document</BlueBtn>}
      </div>
      ...
      {step==="upload"&&(
        <div style={{maxWidth:540,margin:"0 auto"}}>
          <Card style={{textAlign:"center",padding:"clamp(32px,6vw,52px)",border:`2px dashed ${G.border}`,cursor:"pointer",boxShadow:"none"}} onClick={()=>fileRef.current.click()}>
            ...
            <h3 style={{fontSize:18,fontWeight:700,color:G.text,marginBottom:8}}>Upload Study Material</h3>
            <p style={{fontSize:14,color:G.muted,marginBottom:24}}>PDF (up to 10 pages) or standard text/md files</p>
            <input ref={fileRef} type="file" accept=".pdf,.txt,.md" style={{display:"none"}} onChange={handleFile}/>
            <BlueBtn onClick={e=>{e.stopPropagation();fileRef.current.click();}}>Select File</BlueBtn>
            {fileName&&<div style={{marginTop:18,display:"inline-block",padding:"4px 14px",borderRadius:99,fontSize:12,fontWeight:600,background:G.greenSoft,color:G.green,border:`1px solid ${G.green}28`}}>✅ Loaded: {fileName}</div>}
          </Card>

          {payload&&(
            <Card style={{marginTop:16}}>
              <p style={{fontSize:13,color:G.muted,marginBottom:8,fontWeight:600}}>1. Choose Your Learning Mode</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18}}>
                <button 
                  onClick={()=>setMode("flashcards")} 
                  style={{
                    padding:"12px",borderRadius:12,cursor:"pointer",fontSize:13,fontWeight:700,
                    background: mode==="flashcards"?G.purpleSoft:"transparent",
                    color: mode==="flashcards"?G.purple:G.muted,
                    border: mode==="flashcards"?`2px solid ${G.purple}`:`1px solid ${G.border}`,
                    transition:"all 0.15s"
                  }}
                >
                  🎴 Active Flashcards
                ...
                <button 
                  ...
                  style={{
                    padding:"12px",borderRadius:12,cursor:"pointer",fontSize:13,fontWeight:700,
                    background: mode==="quiz"?G.blueSoft:"transparent",
                    color: mode==="quiz"?G.blue:G.muted,
                    border: mode==="quiz"?`2px solid ${G.blue}`:`1px solid ${G.border}`,
                    transition:"all 0.15s"
                  }}
                >
                  📝 Evaluation Quiz
              ...
              <p style={{fontSize:13,color:G.muted,marginBottom:8,fontWeight:600}}>2. Elements to Generate</p>
              <div style={{display:"flex",gap:8,marginBottom:18,flexWrap:"wrap"}}>
                {[3,5,8,10].map(n=><button key={n} onClick={()=>setNumQ(n)} style={{background:numQ===n?G.blueSoft:"transparent",color:numQ===n?G.blue:G.muted,border:numQ===n?`1.5px solid rgba(37,99,235,0.25)`:`1px solid ${G.border}`,borderRadius:99,padding:"7px 18px",fontSize:13,fontWeight:600,cursor:"pointer"}}>{n} Items</button>)}
              </div>
              
              <BlueBtn full onClick={generate} style={{padding:"12px"}}>Process Document with AI →</BlueBtn>
            </Card>
          )}
        </div>
      )}
      ...
      {step==="loading"&&(
        <div style={{textAlign:"center",padding:"80px 20px"}}>
          ...
          <h3 style={{fontSize:20,fontWeight:700,color:G.text,marginBottom:8}}>Structuring your content...</h3>
          <p style={{fontSize:14,color:G.muted}}>Generating customized study materials for your deck</p>
        </div>
      )}

      {/* RENDER ACTIVE FLASHCARDS MODULE */}
      {step==="flashcards" && cards.length > 0 && (
        <div style={{maxWidth:540,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            <p style={{fontSize:13,color:G.muted}}>Card {currentCardIdx+1} of {cards.length} · {fileName}</p>
            <Badge color={G.purple}>Mastered: {masteredCount}/{cards.length}</Badge>
          </div>
            ...
            <div className="flashcard-perspective" onClick={()=>setFlipped(!flipped)}>
              <div className={`flashcard-inner ${flipped ? "is-flipped" : ""}`}>
                ...
                <div className="flashcard-front">
                  <span style={{fontSize:11,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:12}}>Question / Concept</span>
                  <p style={{fontSize:16,fontWeight:600,color:G.text,lineHeight:1.6}}>{cards[currentCardIdx].front}</p>
                  <p style={{fontSize:12,color:G.blue,marginTop:20,fontWeight:600}}>🔄 Click to flip card</p>
                </div>

                {/* Back Side */}
                <div className="flashcard-back">
                  <span style={{fontSize:11,fontWeight:700,color:G.purple,textTransform:"uppercase",letterSpacing:1,marginBottom:12}}>Explanation</span>
                  <p style={{fontSize:15,color:G.text,lineHeight:1.6}}>{cards[currentCardIdx].back}</p>
                  <p style={{fontSize:12,color:G.muted,marginTop:20}}>🔄 Click to show question</p>
                </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:12,alignItems:"center",marginBottom:20}}>
            <button 
              ...
              onClick={()=>{setCurrentCardIdx(p=>p-1); setFlipped(false);}}
              style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:12,padding:12,cursor:"pointer",fontWeight:600,color:currentCardIdx===0?G.muted:G.text}}
            >
              ← Previous
            </button>

            <button 
              onClick={()=>toggleMastered(currentCardIdx)}
              style={{
                background: masteredCards[currentCardIdx] ? G.green : "transparent",
                color: masteredCards[currentCardIdx] ? "#fff" : G.text,
                border: `1.5px solid ${masteredCards[currentCardIdx] ? G.green : G.border}`,
                borderRadius:99, padding:"10px 20px", cursor:"pointer", fontWeight:700, fontSize:13
              }}
            >
              {masteredCards[currentCardIdx] ? "✓ Mastered" : "⭐ Mark Mastered"}
            </button>

            <button 
              disabled={currentCardIdx === cards.length - 1}
              onClick={()=>{setCurrentCardIdx(p=>p+1); setFlipped(false);}}
              style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:12,padding:12,cursor:"pointer",fontWeight:600,color:currentCardIdx===cards.length-1?G.muted:G.text}}
            >
              Next →
            </button>
          </div>
          ...
          <Card style={{textAlign:"center",padding:14}}>
            <p style={{fontSize:12,color:G.muted}}>Tip: Mastering all cards increases your academic daily streak!</p>
          </Card>
        </div>
      )}

      {/* RENDER ACTIVE MCQS MODULE */}
      {step==="quiz" && (
        <div style={{maxWidth:680,margin:"0 auto"}}>
          {revealed&&(
            <Card style={{textAlign:"center",marginBottom:20,background:score/questions.length>=0.7?G.greenSoft:G.redSoft,border:`1px solid ${score/questions.length>=0.7?`${G.green}28`:`${G.red}28`}`}}>
              ...
              <p style={{fontSize:16,fontWeight:600,color:G.text,marginTop:6}}>{score/questions.length>=0.8?"Top Performance! 🏆":score/questions.length>=0.6?"Good progress! ✅":"Review your materials again 📚"}</p>
              <p style={{fontSize:13,color:G.muted,marginTop:4}}>{Math.round((score/questions.length)*100)}% accuracy score</p>
            </Card>
          )}

          {!revealed&&(
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
              <p style={{fontSize:13,color:G.muted}}>{questions.length} Assessment points · {fileName}</p>
              <BlueBtn small onClick={submitQuiz}>Verify Submissions →</BlueBtn>
            </div>
          )}

          {questions.map((q,i)=>(
            <Card key={i} style={{marginBottom:14}}>
              <p style={{fontSize:14,fontWeight:600,color:G.text,marginBottom:14}}><span style={{color:G.blue,marginRight:8}}>Question {i+1}:</span>{q.question}</p>
              {q.options.map((opt,j)=>{
                ...
                return(
                  <div key={j} onClick={()=>!revealed&&setAnswers(a=>({...a,[i]:opt}))} style={{padding:"11px 16px",borderRadius:10,marginBottom:8,cursor:revealed?"default":"pointer",background:bg,border,fontSize:13,color:G.text,display:"flex",justifyContent:"space-between",alignItems:"center",transition:"all 0.15s"}}>
                    <span>{opt}</span>
                    {revealed&&cor&&<span style={{color:G.green,fontWeight:700}}>✓ Correct</span>}
                    {revealed&&sel&&!cor&&<span style={{color:G.red,fontWeight:700}}>✗ Mistake</span>}
                  </div>
                );
              })}
            </Card>
          ))}
          {!revealed&&<BlueBtn full onClick={submitQuiz} style={{padding:"13px"}}>Verify Submissions</BlueBtn>}
        </div>
      )}
    </div>
  );
...
function SectionTeaser({log,toast,streak,setStreak,setSection}) {
  ...
  function check(){
    ...
    if(correct){setScore(s=>s+1);setStreak(s=>s+1);toast("🎉 Accurate! Streak point added!",G.green);log("brainteaser","Answered Brain Teaser accurately",G.green);}
    else{toast("Wrong guess.",G.orange);log("brainteaser","Submitted answer for Brain Teaser",G.gold);}
  }
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <PageHeader onBack={()=>setSection("dashboard")} title="🧠 Focus Puzzles"/>
        <div style={{display:"flex",gap:8}}>
          <Badge color={G.green}>Correct: {score}/{total}</Badge>
          <Badge color={G.orange}>Streak: {streak}</Badge>
        </div>
      </div>
      ...
      <div style={{maxWidth:580,margin:"0 auto"}}>
        <Card style={{textAlign:"center",padding:"clamp(28px,6vw,48px) clamp(20px,5vw,40px)"}}>
          ...
          {hint&&<div style={{background:G.blueSoft,border:`1px solid rgba(37,99,235,0.2)`,borderRadius:10,padding:"10px 16px",marginBottom:18,color:G.blue,fontSize:13}}>💡 Clue: Commences with "{q.a[0]}", containing {q.a.length} letters.</div>}
          {!revealed?(
            <>
              <div style={{display:"flex",gap:10,maxWidth:380,margin:"0 auto 14px",flexWrap:"wrap"}}>
                <input style={{flex:1,background:G.bg,border:`1.5px solid ${G.border}`,borderRadius:10,padding:"11px 14px",fontSize:14,color:G.text,outline:"none",textAlign:"center"}} placeholder="Submit guess..." value={guess} onChange={e=>setGuess(e.target.value)} onKeyDown={e=>e.key==="Enter"&&check()}/>
                <BlueBtn onClick={check} style={{padding:"11px 24px"}}>Verify</BlueBtn>
              </div>
              <GrayBtn onClick={()=>setHint(true)} style={{fontSize:13}}>💡 Request Clue</GrayBtn>
            </>
          ):(
            <>
              <div style={{padding:"16px 24px",borderRadius:12,background:correct?G.greenSoft:G.redSoft,border:`1px solid ${correct?G.green+40:G.red+40}`,marginBottom:20}}>
                <p style={{fontSize:15,fontWeight:700,color:correct?G.green:G.red}}>{correct?"🎉 Precise submission!":"❌ Incorrect submission!"}</p>
                <p style={{fontSize:16,color:G.text,marginTop:5}}>Correct term: <strong style={{color:G.blue}}>{q.a}</strong></p>
              </div>
              <BlueBtn onClick={next} style={{padding:"11px 28px"}}>Next Puzzle →</BlueBtn>
            </>
          )}
        </Card>

        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginTop:16}}>
          {[{l:"Resolved",v:score,c:G.green,i:"✅"},{l:"Attempted",v:total,c:G.blue,i:"🎯"},{l:"Rate",v:total?`${Math.round((score/total)*100)}%`:"—",c:G.gold,i:"📊"}].map(s=>(
...
/* ═══════════════════════════════════════════════════════════════
   LOBBY & STREAK LEADERBOARD SECTION
═══════════════════════════════════════════════════════════════ */
function SectionStreaks({friends,setFriends,streak,log,toast,setSection,users=[],user}) {
  ...
  // Handles dynamic "real-life" interaction with leaderboard elements
  function handlePoke(friendId, friendName) {
    setFriends(p => p.map(f => {
      if (f.id === friendId) {
        toast(`Streak poke sent to ${friendName}! ⚡`);
        log("streak", `Sent dynamic poke warning to ${friendName}`, G.purple);
        return { ...f, status: "Poked! Reviewing lessons." };
      }
      return f;
    }));
  }

  function handleBoost(friendId, friendName) {
    setFriends(p => p.map(f => {
      if (f.id === friendId) {
        toast(`Study Boost applied to ${friendName}! 🛡️`);
        log("streak", `Boosted ${friendName}'s study retention`, G.green);
        return { ...f, boosted: true, streak: f.streak + 1, status: "Active with Study Boost" };
      }
      return f;
    }));
  }
  ...
  async function send(){
    if(!iEmail||!iName){toast("Fill out all invite fields.",G.red);return;}
    setSent(p=>[...p,{id:uid(),name:iName,email:iEmail,status:"sending"}]);
    setIName("");setIEmail("");setOpen(false);
    
    const ok = await sendInviteEmail({toEmail:iEmail, toName:iName, fromName:"ApexEduNexus Peer Network"});
    setSent(p=>p.map(s=>s.email===iEmail?{...s,status:ok?"sent":"failed"}:s));
    log("streak",`Invited collaborator ${iName} to the workspace`,G.orange);
    toast(ok?`Workspace invite dispatched to ${iEmail}! 📧`:`Invite cached successfully! Configure EmailJS to initiate automated relays.`, ok?G.orange:G.gold);
  }

  // Populate dynamic active lobby metrics
  const registeredEntries = users
    ...
    .map(u => ({
      ...
      status: "Reviewing Study Guides",
      boosted: false
    }));

  const board = [
    {id:"me", name:"You", streak, initials: (user?.name||"You").split(" ").map(p=>p[0]).join("").slice(0,2).toUpperCase()||"YO", isMe:true, status: "Working in Hub", boosted: false},
    ...registeredEntries,
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <PageHeader onBack={()=>setSection("dashboard")} title="🔥 Workspace Lobby & Leaderboard"/>
        <BlueBtn small onClick={()=>setOpen(true)} style={{background:G.orange,boxShadow:"none"}}>✉️ Invite Classmate</BlueBtn>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))",gap:16,marginBottom:24}}>
        <Card style={{background:"linear-gradient(135deg,#FFF7ED,#FFFBF5)",border:`1px solid ${G.orange}28`,display:"flex",alignItems:"center",gap:20,padding:20}}>
          <div style={{fontSize:48}}>🔥</div>
          <div>
            <div style={{fontSize:38,fontWeight:900,color:G.orange}}>{streak} Days</div>
            <p style={{fontSize:14,fontWeight:600,color:G.text}}>Active Learning Streak</p>
            <p style={{fontSize:12,color:G.muted}}>Complete flashcards or resolve focus riddles to protect your standing.</p>
          </div>
        </Card>

        <Card style={{background:"linear-gradient(135deg,#F0FDF4,#F9FEFB)",border:`1px solid ${G.green}28`,display:"flex",alignItems:"center",gap:20,padding:20}}>
          <div style={{fontSize:48}}>🏫</div>
          <div>
            <div style={{fontSize:22,fontWeight:900,color:G.green}}>Study Lobby</div>
            <p style={{fontSize:13,color:G.muted,marginTop:4}}>Manage your academic peer network. Interact with classmates to keep your group motivated.</p>
          </div>
        </Card>
      </div>

      <Card style={{marginBottom:24}}>
        <p style={{fontSize:12,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:16}}>🏆 Real-Time Standings & Lobby Interactions</p>
        <div style={{display:"grid",gap:10}}>
          {board.map((f,i)=>(
            <div key={f.id} style={{display:"flex",alignItems:"center",gap:12,padding:"14px",background:i===0?"rgba(245,158,11,0.05)":G.bg,borderRadius:12,border:`1px solid ${i===0?"rgba(245,158,11,0.2)":G.border}`,flexWrap:"wrap"}}>
              <span style={{fontSize:18,width:26,flexShrink:0,textAlign:"center"}}>{medals[i]||`#${i+1}`}</span>
              <div style={{width:40,height:40,borderRadius:"50%",background:f.isMe?G.green:G.blue,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:13,fontWeight:700,flexShrink:0}}>
                {f.initials}
              </div>
              <div style={{flex:1,minWidth:140}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <p style={{fontSize:14,fontWeight:700,color:G.text}}>{f.name}{f.isMe?" (You)":""}</p>
                  {f.boosted && <Badge color={G.green}>🛡️ Boosted</Badge>}
                </div>
                <p style={{fontSize:12,color:G.muted,marginTop:2}}>✏️ Activity: <strong style={{color:G.text}}>{f.status}</strong></p>
              </div>

              <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                <span style={{fontSize:15,fontWeight:800,color:G.orange,marginRight:8}}>🔥 {f.streak}</span>
                {!f.isMe && (
                  <>
                    <button 
                      onClick={()=>handlePoke(f.id, f.name)}
                      style={{background:"none",border:`1.5px solid ${G.border}`,borderRadius:8,padding:"6px 10px",fontSize:11,fontWeight:600,color:G.text,cursor:"pointer"}}
                      title="Poke classmate to keep their streak alive"
                    >
                      ⚡ Poke
                    </button>
                    <button 
                      disabled={f.boosted}
                      onClick={()=>handleBoost(f.id, f.name)}
                      style={{background:G.blueSoft,border:`1.5px solid rgba(37,99,235,0.15)`,borderRadius:8,padding:"6px 10px",fontSize:11,fontWeight:700,color:G.blue,cursor:"pointer",opacity:f.boosted?0.6:1}}
                    >
                      🛡️ Boost
                    </button>
                  </>
      ...
      {sent.length>0&&(
        <Card>
          <p style={{fontSize:12,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:14}}>📨 Peer Invites Outstanding</p>
          {sent.map(s=>(
            <div key={s.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${G.border}`}}>
              ...
              <Badge color={G.orange}>Pending Response</Badge>
            </div>
          ))}
        </Card>
      )}
      ...
      {open&&(
        <Modal title="✉️ Invite Collaborator" onClose={()=>setOpen(false)}>
          <p style={{fontSize:13,color:G.muted,marginBottom:18,marginTop:-12}}>Add a peer to your group space to track comparative learning objectives.</p>
          <div style={{display:"grid",gap:14}}>
            <Input label="Name *" placeholder="e.g. Samuel Patterson" value={iName} onChange={e=>setIName(e.target.value)}/>
            <Input label="Institutional Email *" type="email" placeholder="sam@uni.edu" value={iEmail} onChange={e=>setIEmail(e.target.value)}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:10,marginTop:20}}>
            <BlueBtn full onClick={send} style={{padding:"12px",background:G.orange,boxShadow:"none"}}>Relay Invite 📧</BlueBtn>
            <GrayBtn onClick={()=>setOpen(false)}>Cancel</GrayBtn>
          </div>
        </Modal>
      )}
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
        <PageHeader onBack={()=>setSection("dashboard")} title="📈 System Activity Logs"/>
        <Badge color={G.blue}>{activity.length} Log events</Badge>
      </div>
      ...
      {activity.length===0&&<p style={{textAlign:"center",color:G.muted,padding:"60px 0"}}>System activity records are empty.</p>}
    </div>
  );
}
...
function SectionNotifs({notif,setNotif,deadlines,email,setEmail,toast,setSection}) {
  ...
  return(
    <div>
      <PageHeader onBack={()=>setSection("dashboard")} title="🔔 Alert Priorities & Configurations"/>
      <div style={{maxWidth:520,display:"grid",gap:16}}>
        <Card>
          <p style={{fontSize:12,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:14}}>📧 Relayed Contacts</p>
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            ...
            <BlueBtn small onClick={()=>{setEmail(em);toast("System address confirmed! ✅",G.green);}}>Update Address</BlueBtn>
          </div>
        </Card>
        <Card>
          <p style={{fontSize:12,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:14}}>⚙️ Warning Configurations</p>
          <label style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer",padding:"12px 0",borderBottom:`1px solid ${G.border}`,marginBottom:16}}>
            ...
            <div><p style={{fontSize:14,fontWeight:600,color:G.text}}>Activate system alerts</p><p style={{fontSize:12,color:G.muted}}>Receive warning notifications ahead of deadlines</p></div>
          </label>
          <p style={{fontSize:13,color:G.muted,marginBottom:10}}>Alert threshold before deadlines occur:</p>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {[1,2,3,5,7].map(d=><button key={d} onClick={()=>setNotif(p=>({...p,days:d}))} style={{background:notif.days===d?G.blueSoft:"transparent",color:notif.days===d?G.blue:G.muted,border:notif.days===d?`1.5px solid rgba(37,99,235,0.25)`:`1px solid ${G.border}`,borderRadius:99,padding:"7px 18px",fontSize:13,fontWeight:600,cursor:"pointer"}}>{d} Days Out</button>)}
          </div>
        </Card>
        <Card>
          <p style={{fontSize:12,fontWeight:700,color:G.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:14}}>📋 Active Task Trackers</p>
          {deadlines.filter(d=>d.notify&&!d.done).map(d=>{
            ...
            return(
              <div key={d.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${G.border}`}}>
                ...
                <Badge color={c}>{dy<=0?"Overdue":`${dy}d Remaining`}</Badge>
              </div>
            );
          })}
          {deadlines.filter(d=>d.notify&&!d.done).length===0&&<p style={{color:G.muted,fontSize:13,textAlign:"center",padding:12}}>No pending warnings present.</p>}
        </Card>
        {notif.enabled&&(
          <div style={{background:"rgba(245,158,11,0.07)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:12,padding:"16px 18px"}}>
            <p style={{fontWeight:700,color:G.gold,marginBottom:6}}>📬 Action Plan Summary</p>
            <p style={{fontSize:13,color:G.muted}}>Alerts routed to: <strong style={{color:G.text}}>{em}</strong></p>
            <p style={{fontSize:13,color:G.muted,marginTop:4}}>relay dispatched {notif.days} days prior to target dates.</p>
          </div>
        )}
      </div>
    </div>
