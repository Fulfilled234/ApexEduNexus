...
/* ═══════════════════════════════════════════════════════════════
   EMAIL JS  — replace the 3 values below with yours from emailjs.com
   SERVICE_ID  : your EmailJS service ID
   TEMPLATE_INVITE : template ID for friend invites
   TEMPLATE_NOTIF  : template ID for deadline alerts  
   PUBLIC_KEY  : your EmailJS public key
═══════════════════════════════════════════════════════════════ */
// ── Account 1: Invites + Deadline alerts ──────────────────────
const EJS = {
  SERVICE_ID:        "service_dytqcsn",
  TEMPLATE_INVITE:   "template_i0rp32p",
  TEMPLATE_NOTIF:    "template_m2mnnnj",
  PUBLIC_KEY:        "K5aAjL4_BMqnY0MWp",
};

// ── Account 2 (new): Welcome emails + Password Reset ──────────
const EJS2 = {
  SERVICE_ID:        "service_j5sng3g",
  TEMPLATE_WELCOME:  "template_rqqxc9h",
  TEMPLATE_RESET:    "template_w67zlcv",
  TEMPLATE_INVITE:   "template_rqqxc9h",
  PUBLIC_KEY:        "-1TGyOJpjK9vY3dPt",
};

async function sendEmail(templateId, params, cfg = EJS) {
  try {
    if (!window.emailjs) { console.warn("EmailJS not loaded"); return false; }
    await window.emailjs.send(cfg.SERVICE_ID, templateId, params, cfg.PUBLIC_KEY);
    return true;
  } catch(e) { console.error("EmailJS error:", e); return false; }
}

async function sendInviteEmail({toEmail, toName, fromName}) {
  return sendEmail(EJS2.TEMPLATE_INVITE, {
    to_email: toEmail,
    to_name:  toName,
    from_name: fromName,
...
async function sendDeadlineAlert({toEmail, toName, deadlineTitle, daysLeft, dueDate}) {
  return sendEmail(EJS.TEMPLATE_NOTIF, {
    to_email:   toEmail,
    to_name:    toName,
    title:      deadlineTitle,
    days_left:  daysLeft,
...
async function sendWelcomeEmail({toEmail, toName}) {
  return sendEmail(EJS2.TEMPLATE_WELCOME, {
    to_email:  toEmail,
    to_name:   toName,
    app_link:  window.location.href,
  }, EJS2);
...
async function sendResetEmail({toEmail, toName, code}) {
  return sendEmail(EJS2.TEMPLATE_RESET, {
    to_email:  toEmail,
    to_name:   toName,
    reset_code: code,
  }, EJS2);
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100dvh",background:G.bg,fontFamily:"'Inter',system-ui,-apple-system,sans-serif",color:G.text}}>
      ...
      {screen==="login"  && <LoginPage  setScreen={setScreen} users={users} registerUser={registerUser} updateUserPassword={updateUserPassword} setUser={u=>{setUser(u);setScreen("app");}} showToast={showToast}/>}
      {screen==="signup" && <SignupPage setScreen={setScreen} users={users} registerUser={registerUser} setUser={u=>{setUser(u);setScreen("app");}} showToast={showToast}/>}
      {screen==="app"    && <AppPage    user={user} users={users} setScreen={setScreen} showToast={showToast}/>}
...
function SectionQuiz({log,toast,setSection}) {
  ...
  async function handleFile(e){
    ...
    try{
      ...
      if(file.type==="application/pdf") setPayload({type:"pdf",b64});
      else {const text=await file.text();setPayload({type:"text",text});}
  ...
  async function generate(){
    ...
    try{
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2000,messages})});
      if(!res.ok) throw new Error(`${res.status}`);
      const data=await res.json();
      const raw=data.content?.map(c=>c.type==="text"?c.text:"").join("").trim().replace(/^```(?:json)?\s*/,"").replace(/\s*```$/,"").trim();
      ...
      console.error(err);setStep("upload");
    }
  }
      ...
      {step==="upload"&&(
        <div style={{maxWidth:520,margin:"0 auto"}}>
          <Card style={{textAlign:"center",padding:"clamp(32px,6vw,52px)",border:`2px dashed ${G.border}`,cursor:"pointer",boxShadow:"none"}} onClick={()=>fileRef.current.click()}>
            ...
            <p style={{fontSize:14,color:G.muted,marginBottom:24}}>PDF or text file — Claude AI will generate quiz questions from it</p>
