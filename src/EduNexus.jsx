  ...
  return (
    <div className="min-h-screen flex flex-col justify-between">
      ...
      <div class="max-w-4xl mx-auto px-6 py-20 text-center relative z-10 my-auto">
        ...
        <h1 class="text-4xl sm:text-6xl font-extrabold text-white mb-6 leading-tight font-display">
      ...
      <footer class="border-t border-slate-900/60 bg-slate-950/20 py-8 px-6 text-center">
        <p class="text-xs text-slate-500">© 2026 ApexEduNexus · Academic Excellence</p>
      </footer>
    </div>
  );
}
...
function AppPage({
  ...
  return (
    <div class="min-h-screen flex flex-col md:flex-row">
      <header class="md:hidden border-b border-slate-800 bg-slate-950 px-6 h-16 flex items-center justify-between sticky top-0 z-50">
        <Logo size={14} />
          ...
          <nav class="flex flex-col gap-1.5">
            {navItems.map(item => (
              ...
              >
                ...
                {item.id === "deadlines" && urgent > 0 && <span class="ml-auto bg-red-600 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">{urgent}</span>}
              </button>
            ))}
          </nav>
        </div>
...
function SectionQuiz({ log, toast, setSection, setStreak, personalKey, usePersonalKey }) {
  ...
  return (
    <div class="space-y-6 animate-fade-in">
      <div class="flex items-center justify-between">
        <PageHeader onBack={() => setSection("dashboard")} title="AI Hub Panel" />
        {step !== "upload" && (
          <button onClick={() => setStep("upload")} class="border border-slate-800 hover:border-slate-700 text-slate-300 text-xs font-bold px-4 py-2.5 rounded-full transition-all">
            Upload Alternate
          </button>
        )}
      </div>
      ...
      {step === "upload" && (
        <div class="max-w-xl mx-auto space-y-4">
          <div onClick={() => fileRef.current.click()} class="border-2 border-dashed border-slate-800 hover:border-slate-700 bg-slate-950/40 p-10 rounded-2xl text-center cursor-pointer transition-all">
            <div class="text-3xl text-slate-500 mb-3">📁</div>
            <p class="text-sm font-bold text-white mb-1">Upload Study Document</p>
            <p class="text-xs text-slate-500 mb-4">Strictly accepts PDF files (Max 4MB)</p>
            <input type="file" ref={fileRef} accept=".pdf" onChange={handleFile} class="hidden" />
            <button class="bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 border border-blue-500/30 text-xs font-bold px-5 py-2.5 rounded-xl transition-all">Choose PDF File</button>
            {fileName && <p class="text-xs text-emerald-400 font-bold mt-4 font-display">Loaded: {fileName}</p>}
          </div>

          {payload && (
            <Card class="bg-slate-950 border border-slate-900 p-5 rounded-2xl space-y-4">
              <div class="grid grid-cols-2 gap-3">
                <button onClick={() => setMode("flashcards")} class={`py-3 rounded-xl text-xs font-bold border transition-all ${mode === "flashcards" ? "bg-purple-500/15 border-purple-500/30 text-purple-400" : "bg-slate-900 border-slate-800 text-slate-400"}`}>
                  🎴 Active Flashcards
                </button>
                <button onClick={() => setMode("quiz")} class={`py-3 rounded-xl text-xs font-bold border transition-all ${mode === "quiz" ? "bg-blue-500/15 border-blue-500/30 text-blue-400" : "bg-slate-900 border-slate-800 text-slate-400"}`}>
                  📝 Evaluation Quiz
                </button>
              </div>

              <div>
                <p class="text-xs font-bold text-slate-400 mb-2">Target Volume</p>
                <div class="flex gap-2">
                  {[3, 5, 8, 10].map(n => (
                    <button key={n} onClick={() => setNumQ(n)} class={`flex-1 py-2 text-xs font-bold rounded-lg border transition-all ${numQ === n ? "bg-blue-500/10 border-blue-500/30 text-blue-400" : "bg-slate-900 border-slate-800 text-slate-400"}`}>{n}</button>
                  ))}
                </div>
              </div>

              <button onClick={generate} class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-all font-display">Generate Workspace Deck</button>
            </Card>
          )}
        </div>
      )}
      ...
      {step === "loading" && (
        <div class="text-center py-20">
          ...
          <p class="text-sm font-semibold text-white">Synthesizing study records...</p>
        </div>
      )}
      ...
      {step === "cards" && cards.length > 0 && (
        <div class="max-w-lg mx-auto space-y-4">
          ...
          <div class="flashcard-perspective" onClick={() => setFlipped(!flipped)}>
            <div class={`flashcard-inner ${flipped ? "is-flipped" : ""}`}>
              <div class="flashcard-front bg-slate-950 border border-slate-900 rounded-2xl flex flex-col justify-between p-6">
                <span class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Question Segment</span>
                <p class="text-sm text-slate-200 leading-relaxed font-semibold">{cards[currentIdx].front}</p>
                <span class="text-[10px] text-blue-400 font-bold">🔄 Flip Card</span>
              </div>
              <div class="flashcard-back bg-blue-950/20 border border-blue-900/30 rounded-2xl flex flex-col justify-between p-6">
                <span class="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Factual Answer</span>
                <p class="text-xs text-slate-200 leading-relaxed font-semibold">{cards[currentIdx].back}</p>
                <span class="text-[10px] text-slate-400 font-bold">🔄 Return Front</span>
              </div>
            </div>
          </div>

          <div class="flex justify-between gap-4">
            <button disabled={currentIdx === 0} onClick={() => { setCurrentIdx(p => p - 1); setFlipped(false); }} class="flex-1 bg-slate-900 border border-slate-800 text-slate-300 py-3 rounded-xl disabled:opacity-50 text-xs font-bold">Previous</button>
            <button onClick={() => toggleMastered(currentIdx)} class={`flex-1 py-3 rounded-xl text-xs font-bold border transition-all ${mastered[currentIdx] ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "border-slate-800 text-slate-300"}`}>
              {mastered[currentIdx] ? "Mastered ✓" : "Mark Mastered"}
            </button>
            <button disabled={currentIdx === cards.length - 1} onClick={() => { setCurrentIdx(p => p + 1); setFlipped(false); }} class="flex-1 bg-slate-900 border border-slate-800 text-slate-300 py-3 rounded-xl disabled:opacity-50 text-xs font-bold">Next</button>
          </div>
        </div>
      )}
      ...
      {step === "quiz" && (
        <div class="max-w-xl mx-auto space-y-4">
          {revealed && (
            <div class="bg-slate-950 border border-slate-900 p-5 rounded-2xl text-center">
              <p class="text-3xl font-black text-white">{questions.filter((q, i) => answers[i] === q.answer).length} / {questions.length}</p>
              <p class="text-xs text-slate-400 mt-2">Evaluation accuracy calculated.</p>
            </div>
          )}

          {questions.map((q, i) => (
            <Card key={i} class="bg-slate-950 border border-slate-900 p-5 rounded-2xl space-y-3">
              <p class="text-sm font-bold text-white"><span class="text-blue-500">Q{i + 1}.</span> {q.question}</p>
              <div class="space-y-2">
                {q.options.map((opt, j) => {
                  const sel = answers[i] === opt;
                  return (
                    <div key={j} onClick={() => !revealed && setAnswers(p => ({ ...p, [i]: opt }))} class={`p-3 rounded-xl text-xs font-medium cursor-pointer border transition-all ${sel ? "bg-blue-500/10 border-blue-500/30 text-blue-400" : "bg-slate-900/40 border-slate-800 text-slate-400"}`}>
                      {opt}
                    </div>
                  );
                })}
          ...
          {!revealed && <button onClick={submitQuiz} class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl transition-all">Submit Answers</button>}
        </div>
      )}
    </div>
  );
...
function SectionStreaks({ streak, setStreak, log, toast, setSection, user }) {
  ...
  async function sendInvite() {
    ...
    toast(`Workspace invite dispatched securely to ${iEmail}!`);
    setOpen(false);
    setIName("");
    setIEmail("");
  }
  ...
  return (
    <div class="space-y-6 animate-fade-in">
      <div class="flex items-center justify-between">
        <PageHeader onBack={() => setSection("dashboard")} title="Lobby & Standings" />
      ...
      <Card class="bg-gradient-to-br from-amber-600/10 to-amber-700/5 border border-amber-500/20 p-5 rounded-2xl flex items-center gap-4">
        <div class="text-3xl">🔥</div>
        <div>
          <h4 class="text-sm font-bold text-white">Your Current Active Streak</h4>
          <p class="text-2xl font-black text-amber-500 mt-1">{streak} Days</p>
          <p class="text-xs text-slate-400 mt-1">Submit flashcards or complete evaluation quizzes daily to retain status.</p>
        </div>
      </Card>

      <div class="space-y-4">
        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider">🏆 Live Global Standing</h3>

        {loading ? (
          <p class="text-xs text-slate-500 text-center py-6">Connecting to database...</p>
        ) : (
          <div class="space-y-2">
            {board.map((f, i) => {
              const isMe = f.id === user?.id;
              return (
                <div key={f.id} class={`p-4 rounded-xl border flex items-center justify-between transition-all ${isMe ? "bg-blue-500/15 border-blue-500/30" : "bg-slate-950 border-slate-900"}`}>
                  <div class="flex items-center gap-3">
                    <span class="text-base font-bold w-6 text-center">{medals[i] || `#${i + 1}`}</span>
                    <div class="w-8 h-8 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center font-bold text-xs text-slate-300">
                      {f.name?.slice(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <p class="text-xs font-bold text-white">{f.name} {isMe && "(You)"}</p>
                      <p class="text-[10px] text-slate-500">{f.email}</p>
                    </div>
                  </div>

                  <div class="flex items-center gap-4">
                    <span class="text-xs font-black text-amber-500">🔥 {isMe ? streak : f.streak}</span>
                    {!isMe && (
                      <div class="flex gap-2">
                        <button onClick={() => handlePoke(f.id, f.name)} class="bg-slate-900 border border-slate-800 text-slate-400 hover:text-white px-2.5 py-1.5 rounded-lg text-[10px] font-bold">
                          Poke
                        </button>
                        <button onClick={() => handleBoost(f.id, f.name)} class="bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 border border-blue-500/25 px-2.5 py-1.5 rounded-lg text-[10px] font-bold">
                          Boost
                        </button>
                      </div>
                    )}
            ...
            {board.length === 0 && <p class="text-xs text-slate-500 text-center">No other classmates are currently active.</p>}
          </div>
        )}
      </div>
      ...
      {open && (
        <Modal title="Invite Academic Partner" onClose={() => setOpen(false)}>
          <div class="space-y-4">
            ...
            <button onClick={sendInvite} class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl transition-all font-display">Send Workspace Invite</button>
          </div>
        </Modal>
      )}
    </div>
  ...
  return (
    <div class="space-y-6 animate-fade-in">
      ...
      <Card class="bg-slate-950 border border-slate-900 p-5 rounded-2xl space-y-4">
        ...
        {activity.length === 0 && <p class="text-xs text-slate-500 text-center">No logs generated this session.</p>}
      </Card>
    </div>
  );
}
    ...
    <div class="min-h-screen flex flex-col justify-center items-center p-6 bg-radial-glow">
      <Card class="w-full max-w-sm bg-slate-950 border border-slate-900 p-8 rounded-2xl space-y-6">
        ...
        <div class="space-y-4">
          ...
          <button onClick={submit} class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl transition-all font-display">Continue to Console</button>
        </div>
    ...
    <div class="min-h-screen flex flex-col justify-center items-center p-6 bg-radial-glow">
      <Card class="w-full max-w-sm bg-slate-950 border border-slate-900 p-8 rounded-2xl space-y-6">
        ...
        <div class="space-y-4">
          ...
          <button onClick={submit} class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl transition-all font-display">Initialize Platform</button>
        </div>
...
const Toast = ({ msg }) => (
  <div class="fixed bottom-6 right-6 z-50 bg-slate-900 border border-slate-800 px-5 py-3 rounded-xl shadow-xl animate-bounce">
