...
/* ═══════════════════════════════════════════════════════════════
   CONSTANTS & UTILS
═══════════════════════════════════════════════════════════════ */
// Nigerian university grading scale (5-point), used by FUTA and most Nigerian institutions —
// NOT the US 4.0 scale this was previously hardcoded to.
const GP = {"A":5,"B":4,"C":3,"D":2,"E":1,"F":0};
const uid = () => (crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2, 9)}`);

// supabase-js v2 does NOT throw on database errors (RLS denials, bad column
// types, etc.) — it resolves with { data: null, error }. Every write in this
// app used to do `await supabase.from(...).insert(...)` inside a try/catch
// and never looked at `error`, so failed writes were invisible: the UI kept
// its optimistic local update, showed a success toast, and then the row was
// simply never in the database — which is exactly why things "saved" but
// vanished on refresh. This wrapper checks the real result and reports it.
async function dbSync(promise, label, toast) {
  try {
    const { error } = await promise;
    if (error) {
      console.error(`${label} failed:`, error.message);
      toast?.(`${label} didn't save to your account (${error.message}). It's on this device only.`, "#EF4444");
      return false;
    }
    return true;
  } catch (e) {
    console.error(`${label} failed:`, e);
    toast?.(`${label} didn't save — no connection to server. It's on this device only.`, "#EF4444");
    return false;
  }
}
...
const UNLIMITED_KEY_LINK = "https://aistudio.google.com/apikey";
...
export default function App() {
  ...
  // PWA install prompt. Chrome/Android fire 'beforeinstallprompt' when the app is
  // installable; we stash the event so a normal in-app button can trigger it later
  // (browsers block calling .prompt() outside of the original event handler unless
  // you hang onto the event like this). Fires 'appinstalled' once actually installed,
  // so the button can hide itself instead of offering to "install" something already installed.
  ...
  const dismissInstallPopup = () => {
    ...
    // Don't nag again for a week once someone taps "Not now" — the button in
    // the sidebar/footer stays available for whenever they change their mind.
    localStorage.setItem("apex_install_dismissed_at", String(Date.now()));
  };
  const showInstallPopup = !!installPrompt && !installDismissed && (() => {
    const last = Number(localStorage.getItem("apex_install_dismissed_at") || 0);
  ...
  const fetchCloudData = async (userId) => {
    try {
      const { data: dlData, error: dlErr } = await supabase.from("edu_deadlines").select("*").eq("user_id", userId);
      if (dlErr) console.error("Loading deadlines failed:", dlErr.message);
      if (dlData) setDeadlines(dlData);

      const { data: cData, error: cErr } = await supabase.from("edu_courses").select("*").eq("user_id", userId);
      if (cErr) console.error("Loading courses failed:", cErr.message);
      if (cData) setCourses(cData);

      const { data: tData, error: tErr } = await supabase.from("edu_todos").select("*").eq("user_id", userId);
  ...
  useEffect(() => {
    const checkUserSession = async () => {
      ...
      if (session?.user) {
        ...
        // No real Supabase session — the previous version of this code trusted a
        // cached "apex_active_user" from localStorage indefinitely, which let the
        // UI show as fully logged in and attempt cloud writes with no valid auth
        // token at all. Every RLS-protected insert/update/delete then failed
        // silently (or, with the error surfacing fix, showed a permission error
        // that looked like a policy problem when the real issue was just an
        // expired/missing session). Clearing the stale cache and treating this
        // as logged-out forces a fresh sign-in, which gets a real session and
        // makes writes actually work again.
        localStorage.removeItem("apex_active_user");
        setUser(null);
        loadLocalCache();
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans selection:bg-purple-200 selection:text-purple-900">
      ...
      {screen === "home" && <HomePage setScreen={setScreen} installPrompt={installPrompt} onInstallClick={handleInstallClick} />}
      ...
      {screen === "app" && (
        <AppPage 
          ...
          showToast={showToast}
          installPrompt={installPrompt} onInstallClick={handleInstallClick}
        />
      )}
      
      {toast && <Toast msg={toast.msg} color={toast.color} />}
      {showInstallPopup && <InstallPopup onInstall={handleInstallClick} onDismiss={dismissInstallPopup} />}
    </div>
  );
}
...
═══════════════════════════════════════════════════════════════ */
function HomePage({ setScreen, installPrompt, onInstallClick }) {
  const features = [
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <footer className="border-t border-gray-100 bg-white py-10 px-6 text-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-[11px] text-gray-300 mt-1">Built for students, by students</p>
          <div className="flex flex-wrap items-center justify-center gap-3 mt-3">
            {installPrompt && (
              <button onClick={onInstallClick} className="flex items-center gap-1.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold px-4 py-2.5 rounded-full transition-all">
            ...
            <a href="https://olajide-michael-s-portfolio.vercel.app/" target="_blank" rel="noreferrer" className="flex items-center gap-1.5 bg-purple-50 hover:bg-purple-100 text-purple-700 text-xs font-bold px-4 py-2.5 rounded-full transition-all">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14c-4.4 0-8 2.2-8 5v1h16v-1c0-2.8-3.6-5-8-5z"/></svg>
              Meet the Developer
            </a>
          </div>
        </div>
    ...
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <aside className={`fixed md:sticky top-16 md:top-0 left-0 z-30 w-72 h-[calc(100vh-64px)] md:h-screen border-r border-gray-100 bg-white p-6 flex flex-col justify-between transform transition-transform duration-200 md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex flex-col gap-6">
          ...
          {installPrompt && (
            <button onClick={onInstallClick} className="flex items-center justify-center gap-2 w-full bg-purple-600 hover:bg-purple-700 text-white px-4 py-3 rounded-xl text-xs font-bold transition-all shadow-sm shadow-purple-200">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5 5-5M12 15V3"/></svg>
              Install App
            </button>
          )}
        ...
        <div className="pt-4 border-t border-gray-100 flex flex-col gap-2">
          ...
          <a href="https://olajide-michael-s-portfolio.vercel.app/" target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full bg-purple-50 hover:bg-purple-100 text-purple-700 px-4 py-3 rounded-xl text-xs font-bold transition-all">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14c-4.4 0-8 2.2-8 5v1h16v-1c0-2.8-3.6-5-8-5z"/></svg>
            Meet the Developer
          </a>
        </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-5">
        ...
        <Bar pct={(numericGPA / 5) * 100} color="#10B981" />
        <p className="text-[11px] text-gray-400 mt-2 mb-4">out of 5.00 maximum</p>
...
function SectionDeadlines({ user, deadlines, setDeadlines, log, toast, setSection }) {
  ...
  async function add() {
    ...
    await dbSync(supabase.from("edu_deadlines").insert([newDeadline]), "Deadline", toast);
  }
  ...
  async function remove(id, title) {
    ...
    await dbSync(supabase.from("edu_deadlines").delete().eq("id", id), "Deadline delete", toast);
  }
          ...
          return (
            <div key={d.id} className="flex items-center gap-4 bg-white border border-gray-100 shadow-sm p-4 rounded-xl">
              <input type="checkbox" checked={d.done} onChange={async () => {
                ...
                await dbSync(supabase.from("edu_deadlines").update({ done: toggledDone }).eq("id", d.id), "Deadline update", toast);
...
function SectionCGPA({ user, courses, setCourses, log, toast, setSection }) {
  ...
  async function add() {
    ...
    await dbSync(supabase.from("edu_courses").insert([newCourse]), "Course", toast);
  }

  async function deleteCourse(id) {
    setCourses(p => p.filter(x => x.id !== id));
    toast("Subject removed.");
    await dbSync(supabase.from("edu_courses").delete().eq("id", id), "Course delete", toast);
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-6 text-white">
        ...
        <Bar pct={(numericGPA / 5) * 100} color="rgba(255,255,255,0.9)" />
        <p className="text-xs text-emerald-50 mt-2">out of 5.00</p>
      </div>
...
function SectionTodos({ user, todos, setTodos, log, toast, setSection }) {
  ...
  async function add() {
    ...
    await dbSync(supabase.from("edu_todos").insert([newTodo]), "Task", toast);
  }

  async function toggle(id, currentDone) {
    const nextDone = !currentDone;
    setTodos(p => p.map(t => t.id === id ? { ...t, done: nextDone } : t));
    await dbSync(supabase.from("edu_todos").update({ done: nextDone }).eq("id", id), "Task update", toast);
  }

  async function remove(id) {
    setTodos(p => p.filter(t => t.id !== id));
    toast("Task deleted permanently.");
    await dbSync(supabase.from("edu_todos").delete().eq("id", id), "Task delete", toast);
  }
...
function SectionQuiz({ log, toast, setSection, setStreak, personalKey, usePersonalKey, user }) {
  ...
  async function handleFile(e) {
    ...
    // Vercel serverless functions hard-cap the request body at ~4.5MB, and
    // base64-encoding a PDF inflates its size by ~33%. A 4MB PDF becomes a
    // ~5.3MB request and gets rejected before it ever reaches Gemini, so the
    // real ceiling has to be lower than the raw function limit.
    if (file.size > 2.5 * 1024 * 1024) {
      toast("File size boundary restriction exceeded (Max 2.5MB PDF).", "#EF4444");
      return;
    }
  ...
  async function generate() {
    ...
    // A per-request random seed nudges Gemini toward a different pick of
    // content/phrasing each time, instead of returning near-identical
    // questions on repeated generations from the same file.
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errBody.error || ("API responded with code " + response.status));
      }
      ...
      {step === "upload" && (
        <div className="max-w-xl mx-auto space-y-4">
          <div onClick={() => fileRef.current.click()} className="border-2 border-dashed border-gray-200 hover:border-purple-200 bg-white p-10 rounded-2xl text-center cursor-pointer transition-all">
            ...
            <p className="text-xs text-gray-400 mb-4">Accepts PDF files (Max 2.5MB)</p>
            ...
            <Card className="p-5 space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {[3, 5, 8, 10, 15, 20, 25, 30].map(n => (
                    <button key={n} onClick={() => setNumQ(n)} className={`flex-1 min-w-[44px] py-2 text-xs font-bold rounded-lg border transition-all ${numQ === n ? "bg-purple-50 border-purple-200 text-purple-700" : "bg-gray-50 border-gray-200 text-gray-500"}`}>{n}</button>
                  ))}
                </div>
              </div>

              {!usePersonalKey && (
                // This is the "get unlimited questions" upsell — deliberately placed right on
                // the quiz panel instead of buried in Settings. UNLIMITED_KEY_LINK below is the
                // click target: point it at your own ad/landing page (it opens in a new tab)
                // before the person lands on Google AI Studio to grab a free key. Clicking it
                // also jumps them into Settings with the personal-key field ready to paste into.
                <a
                  href={UNLIMITED_KEY_LINK}
                  target="_blank"
                  rel="noreferrer"
                  onClick={() => setSection("settings")}
                  className="flex items-center justify-between gap-3 bg-gradient-to-r from-purple-600 to-purple-500 text-white p-4 rounded-xl hover:from-purple-700 hover:to-purple-600 transition-all"
                >
                  <div>
                    <p className="text-xs font-black">Get your free API key for unlimited questions</p>
                    <p className="text-[10px] text-purple-100 mt-0.5">Shared free tier is capped daily — your own key removes the limit.</p>
                  </div>
...
function SectionStreaks({ streak, setStreak, log, toast, setSection, user }) {
  ...
  async function sendInvite() {
    ...
    try {
      ...
      toast(`Couldn't send invite (${e.message}).`, "#EF4444");
    }
    setOpen(false);
    setIName("");
    setIEmail("");
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="bg-purple-50 border border-purple-100 p-4 rounded-xl">
                ...
                <ol className="text-[10px] text-gray-500 space-y-1.5 list-decimal pl-4">
                  <li>Visit <a href={UNLIMITED_KEY_LINK} target="_blank" rel="noreferrer" className="text-purple-600 font-bold hover:underline">Google AI Studio</a>.</li>
...
function SignupPage({ setScreen, onLogin, showToast }) {
  ...
  async function submit() {
    ...
    try {
      ...
      }).then(r => { if (!r.ok) r.json().then(b => console.error("Welcome email failed:", b.error)); })
...
const InstallPopup = ({ onInstall, onDismiss }) => (
  <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-6 md:w-96 z-50 bg-white border border-gray-100 shadow-2xl rounded-2xl p-4 flex items-center gap-3 animate-fade-in">
    <img src="/icon-192.png" alt="" className="w-11 h-11 rounded-xl shrink-0" />
    <div className="min-w-0 flex-1">
      <p className="text-sm font-black text-gray-900">Install ApexEduNexus</p>
      <p className="text-[11px] text-gray-400 mt-0.5">Quicker access from your home screen, and it works offline.</p>
    </div>
    <div className="flex flex-col gap-1.5 shrink-0">
      <button onClick={onInstall} className="bg-purple-600 hover:bg-purple-700 text-white text-[11px] font-bold px-3.5 py-2 rounded-lg transition-all">Install</button>
      <button onClick={onDismiss} className="text-[10px] text-gray-400 hover:text-gray-600 font-semibold">Not now</button>
    </div>
  </div>
);
